 ...
 */
class WorkflowWebRTC[M, T <: Peer.ModelTransformPeerSignaler[M]](signaler: T, config:RTCConfiguration) extends WebRTC[M, T](signaler,config) {
  ...
  def initReceivers(p:Promise[Peer.Room]) = {
    ...
    signaler.receivers.push({
      ...
      case o:Peer.Offer if o.local.id != signaler.localPeer.id =>
        ...
        localStreamFuture.foreach { stream =>
          println("Got Local Stream...")
  ...
  initReceivers(Promise[Peer.Room]())
